...
export default function Absensi() {
  ...
  const handleSubmit = () => {
    ...
    if (belumDipilih)
      return alert("Masih ada anggota yang belum dipilih statusnya");
          ...
          {anggota.map((a) => (
            <div key={a._id} className="absensi-card">
              ...
              <select
                value={absensi[a._id]}
                onChange={(e) =>
                  setAbsensi({ ...absensi, [a._id]: e.target.value })
                }
                className={
                  absensi[a._id] === "Hadir"
                    ? "status-hadir"
          ...
          <div className="rekap-container">
            ...
            <div className="absensi-grid">
              {rekap.data.map((r, i) => (
                <div
                  key={i}
                  className={`absensi-card ${
                    r.status === "Hadir" ? "status-hadir" : "status-tidak"
                  }`}
                >
                  <h3>{r.nama}</h3>
                  <p>{r.status}</p>
